...
const NUMBER_RULE='בכל תשובה שמכילה מספרים, יש להקיף כל מספר ברווח, פסיק ורווח: לפני המספר ואחריו. לדוגמה: , 25 , .';
const SYSTEM=[FILTER,NUMBER_RULE,'כאשר נדרש מידע עדכני, חפש אותו בזמן אמת באמצעות Google Search לפני מתן התשובה.',process.env.AI_SYSTEM_INSTRUCTION||''].filter(Boolean).join('\n\n');
