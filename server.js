...
async function keypadOrNull(call,prompt){const v=await call.read([{type:'text',data:prompt}], 'Digits', {min_digits:1,max_digits:20,sec_wait:2,block_asterisk_key:false,allow_empty:true,empty_val:'',removeInvalidChars:true});const digits=String(v||'').trim();if(!digits)return null;if(digits==='*')return 'hangup';return '/'+digits.replace(/[^0-9/]/g,'');}
